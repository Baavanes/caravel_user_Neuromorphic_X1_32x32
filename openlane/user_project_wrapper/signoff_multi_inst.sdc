...
if {[llength $mprj_cell]} {
    set_disable_timing $mprj_cell
    set_false_path -through [get_pins -hierarchical mprj/*]
}

# ----------------------------
# Macro-wrapper timing cleanup
# ----------------------------
if {[llength [get_clocks wb_clk_i]] == 0} {
  create_clock -name wb_clk -period 25 [get_ports wb_clk_i]
}

foreach inst {mprj mprj1} {
  if {[llength [get_cells $inst]]} {
    set_disable_timing [get_cells $inst]
    set_false_path -through [get_pins $inst/*]
  }
}

set_false_path -from [get_ports *] -to [get_ports *]
set_false_path -from [get_ports *] -to [all_registers]
set_false_path -from [all_registers] -to [get_ports *]
